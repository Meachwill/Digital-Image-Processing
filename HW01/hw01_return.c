...
int main()
{
    ...
    //image in
    image_org = fopen("test_image_r0.5.bmp", "rb");
    //hitogram out
    gray = fopen("gray_dis.txt", "w+t");
    //fread binary data
    ...
    image_new_output = fopen("re_test_imager0.5.bmp", "wb");
    fwrite(IMAGE_NEW.header_TEST, 1, sizeof(image_out), image_new_output);
    fclose(image_new_output);
    image_hisogram = fopen("re_test_image_histogram0.5.txt", "w+t");
    for (int i = 0; i < 262144; i++)
    {
